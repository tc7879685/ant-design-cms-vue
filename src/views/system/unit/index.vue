...
<script>
  ...
  import {queryUnitTreeList, searchByKeywords, deleteByDepartId} from '@/api/api'
  ...
  export default {
    ...
    methods: {
      ...
      loadTree() {
        ...
        that.treeData = []
        that.departTree = []
        queryUnitTreeList().then((res) => {
          if (res.success) {
            that.treeData = res.result.treeData;
            that.departTree = res.result.treeData;
            that.iExpandedKeys =  res.result.ids;
            this.loading = false
          }
        })
      },
